...
__global__ void reshape_and_cache_flash_kernel(
  ...
  for (int i = threadIdx.x; i < n; i += blockDim.x) {
    ...
    if constexpr (kv_dt == Fp8KVCacheDataType::kAuto) {
      ...
      key_cache[tgt_key_value_idx] = fp8::scaled_convert<cache_t, scalar_t, kv_dt>(tgt_key, kv_scale);
      value_cache[tgt_key_value_idx] = fp8::scaled_convert<cache_t, scalar_t, kv_dt>(tgt_value, kv_scale);
    }
  }
}
}  // namespace vllm
